...
class DictionaryTurnPatternTree(QTreeView):
    ...
    def display_turn_patterns_for_variation(
        ...
        structural_variation_path = os.path.join(
            pattern_folder, f"{structural_variation}.json"
        )
        ...
        if os.path.exists(structural_variation_path):
            with open(structural_variation_path, "r") as file:
                ...
                for turn_pattern in sequence_data.get("turn_patterns", []):
                    ...
                    item.setData(
                        turn_pattern, Qt.ItemDataRole.UserRole
                    )  # Store the full turn pattern data
    ...
    def on_turn_pattern_double_clicked(self, index: QModelIndex) -> None:
        ...
        turn_pattern_str = (
            item.text()
        )  # Assuming the turn pattern string is directly the item text.
        ...
        self.dictionary.sequence_widget.current_sequence_json_handler.update_sequence_with_pattern(
            sequence
        )

        # Optionally, show a notification or update the UI to reflect the change.
        self.dictionary.indicator_label.show_indicator(
            "Turn pattern applied to the current sequence."
        )
