...
class VtgDirWidget(TurnsBoxWidget):
    ...
    def _set_default_vtg_dir(self):
        has_turns = any(
            motion.turns > 0
            for pictograph in self.turns_box.turns_panel.scroll_area.scroll_area.pictographs.values()
    ...
    def _set_prop_rot_dir(self, prop_rot_dir: VtgDirections) -> None:
        for (
            pictograph
        ) in self.turns_box.turns_panel.scroll_area.scroll_area.pictographs.values():
            for motion in pictograph.motions.values():
                if motion.motion_type in [DASH, STATIC]:
                    if motion.color == self.turns_box.color:
                        ...
                        motion.pictograph.state_updater.update_pictograph(
                            pictograph_dict
                        )
        if prop_rot_dir:
