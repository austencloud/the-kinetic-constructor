...
if TYPE_CHECKING:
    from widgets.turns_box.turns_box_widgets.turns_widget.turns_widget import TurnsWidget
...
class TurnsButtonManager:
    ...
    def setup_adjust_turns_buttons(self) -> None:
        ...
        self.adjust_turns_buttons = [self._create_and_add_button(adj, text) for adj, text in self.adjustments]

    def _setup_button_frames(self) -> None:
        self.left_spacer_frame = QFrame()
        self.right_spacer_frame = QFrame()
        spacer_height = int(self.turns_widget.turns_box.turns_panel.filter_tab.section.width() / 28)
    ...
    def _create_and_add_button(self, adjustment: AdjustmentNums, text: AdjustmentStrs) -> AdjustTurnsButton:
        ...
        button.clicked.connect(lambda _, adj=adjustment: self.turns_widget.display_manager.adjust_turns(adj))

        layout = self.negative_buttons_hbox_layout if adjustment < 0 else self.positive_buttons_hbox_layout
