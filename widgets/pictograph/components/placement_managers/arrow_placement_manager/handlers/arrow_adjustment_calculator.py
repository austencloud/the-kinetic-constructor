...
class ArrowAdjustmentCalculator:
    ...
    def get_adjustment(self, arrow: Arrow) -> QPointF:
            ...
            if self.pm.pictograph.letter in special_placements:
                special_adjustment = self.pm.adjustment_calculator.get_adjustment_for_letter(
    ...
    def calculate_turns_tuple(self, arrow: Arrow, adjustment_key: str):
        placements = (
            self.pm.special_positioner.placement_manager.pictograph.main_widget.special_placements
        )
        letter_data: Dict[str, Dict] = placements.get(
            self.pm.special_positioner.pictograph.letter, {}
        )

        # Get the regular adjustment
        adjustment = letter_data.get(adjustment_key, {}).get("adjustment")

        # Check for special rotation keys with a pattern
        special_rotation = self._find_special_rotation(
            letter_data.get(adjustment_key, {})
        )
