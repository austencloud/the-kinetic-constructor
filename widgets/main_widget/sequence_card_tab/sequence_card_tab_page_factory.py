...
class SequenceCardTabPageFactory:
    ...
    def create_page(self) -> QFrame:  # Return QFrame instead of QGridLayout
        # Start a new row layout if necessary (only two pages per row)
        if (
            not self.sequence_card_tab.scroll_layout.count()
            or self.sequence_card_tab.scroll_layout.itemAt(
                self.sequence_card_tab.scroll_layout.count() - 1
            )
            .layout()
            .count()
            >= 2
            ...
            current_row_layout = self.sequence_card_tab.scroll_layout.itemAt(
                self.sequence_card_tab.scroll_layout.count() - 1
            ).layout()
