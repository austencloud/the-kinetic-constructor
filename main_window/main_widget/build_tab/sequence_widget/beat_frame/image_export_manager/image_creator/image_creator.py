...
from main_window.main_widget.build_tab.sequence_widget.beat_frame.image_export_manager.image_creator.reversal_processor import (
    ReversalProcessor,
)
    ...
    def _draw_additional_info(
        ...
        if options["add_difficulty_level"]:
            difficulty_level = self.export_manager.main_widget.sequence_level_evaluator.get_sequence_difficulty_level(
                self.export_manager.beat_frame.json_manager.sequence_loader_saver.load_current_sequence_json()
            )
        ...
        for beat_view in filled_beats:
            beat_view.beat_number_item.setVisible(options["add_beat_numbers"])
