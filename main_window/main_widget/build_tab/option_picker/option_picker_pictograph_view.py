...
class OptionPickerPictographView(BorderedPictographView):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if self._ignoreMouseEvents or self._ignoreNextMousePress:
            ...
            self.pictograph.main_widget.build_tab.sequence_constructor.option_picker.manual_builder.option_click_handler.on_option_clicked(
                self.pictograph
            )
        QApplication.restoreOverrideCursor()

    def enterEvent(self, event: QEvent) -> None:
        from main_window.main_widget.build_tab.sequence_widget.graph_editor.pictograph_container.GE_pictograph_container import (
    ...
    def calculate_view_size(self) -> int:
        ...
        calculated_width = int(
            (self.pictograph.main_widget.build_tab.sequence_constructor.width() / 8)
            - spacing
        )
        ...
        view_width = (
            calculated_width
            if calculated_width
            < self.pictograph.main_widget.build_tab.sequence_constructor.height() // 8
            else self.pictograph.main_widget.build_tab.sequence_constructor.height()
            // 8
        )
