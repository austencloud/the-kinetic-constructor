...
class OptionPickerPictographView(BorderedPictographView):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if self._ignoreMouseEvents or self._ignoreNextMousePress:
            ...
            self.pictograph.main_widget.build_tab.manual_builder.option_picker.manual_builder.option_click_handler.on_option_clicked(
                self.pictograph
            )
    ...
    def calculate_view_size(self) -> int:
        ...
        calculated_width = int(
            (self.pictograph.main_widget.build_tab.manual_builder.width() / 8) - spacing
        )

        view_width = (
            calculated_width
            if calculated_width
            < self.pictograph.main_widget.build_tab.manual_builder.height() // 8
            else self.pictograph.main_widget.build_tab.manual_builder.height() // 8
        )
