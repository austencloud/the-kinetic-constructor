...
class OptionScrollDisplayManager:
    SPACING = 5
    ...
    def order_and_display_pictographs(self) -> None:
        for letter_type in LetterType:
            ...
            for index, (_, pictograph) in enumerate(ordered_pictographs.items()):
    ...
    def add_pictograph_to_layout(self, pictograph: BasePictograph, index: int) -> None:
        ...
        section = self.scroll_area.sections[letter_type]
        if section:
    ...
    def add_pictograph_to_section(self, pictograph: BasePictograph):
        """Add a pictograph to its corresponding section layout."""
        letter_type = LetterType.get_letter_type(pictograph.letter)
        section: OptionPickerSectionWidget = self.scroll_area.sections[letter_type]
        if section:
