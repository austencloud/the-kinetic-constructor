...
from main_window.main_widget.sequence_widget.graph_editor.adjustment_panel.turns_box.turns_widget.turns_display_frame.turns_display_frame import TurnsDisplayFrame
from main_window.main_widget.sequence_widget.graph_editor.adjustment_panel.turns_box.turns_widget.turns_updater import TurnsUpdater
...
class TurnsWidget(QWidget):
    ...
    def update_turns_display(self, motion: "Motion", new_turns: str) -> None:
        ...
        if self.turns_box.matching_motion.motion_type in [PRO, ANTI, FLOAT]:
            self.display_frame.decrement_button.setEnabled(
                new_turns not in ["fl"]
            )
        else:
