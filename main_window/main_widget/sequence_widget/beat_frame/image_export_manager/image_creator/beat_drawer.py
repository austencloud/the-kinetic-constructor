        ...
        for row in range(row_count + 1):
            for col in range(start_col, column_count):
                if beat_number < len(filled_beats):
                    ...
                    print(
                        f"Blue motion dict for beat {beat_number}: {beat_view.beat.blue_motion.motion_data}"
                    )
                    print(
                        f"Red motion dict for beat {beat_number}: {beat_view.beat.red_motion.motion_data}"
                    )
                    print("blue motion turns: ", beat_view.beat.blue_motion.turns)
                    print(
                        f"Red motion turns: {beat_view.beat.red_motion.turns}"
                    )  # ✅ Correct value                    # print the motion itself
