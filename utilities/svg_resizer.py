...
class SvgResizer(QMainWindow):
    ...
    def select_svg_files(self) -> None:
        ...
        file_dialog.setPictograph(QFileDialog.Pictograph.ShowDirsOnly)  # Show only directories
    ...
    def load_thumbnails(self, svg_file_paths) -> None:
        ...
        for path in svg_file_paths:
            ...
            svg_item.setPos(x_offset, 0)  # Positions each item next to the previous one
            scene.addItem(svg_item)

            # Add border for each SVG item
            bbox = svg_item.boundingRect()
            border_rect: QGraphicsRectItem = QGraphicsRectItem(
                QRectF(0, 0, bbox.width(), bbox.height()), svg_item
            )
    ...
    def trim_excess_space(self) -> None:
        try:
            ...
            QMessageBox.information(
                self, "Success", "Excess space trimmed successfully."
            )
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An error occurred: {e}")
