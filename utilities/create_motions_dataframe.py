...
for position_pair, letters_info in data.items():
    ...
    for blue_start_orientation in ['in', 'out']:  # Iterate over both start orientations for blue
        for red_start_orientation in ['in', 'out']:  # Iterate over both start orientations for red
            for letter_data in letters_info:
                ...
                if len(motion_pairs) >= 2 and all(isinstance(item, dict) for item in motion_pairs[:2]):
                    ...
                    blue_end_orientation = get_end_orientation(blue_start_orientation, blue_motion['motion_type'], blue_motion['turns'])
                    red_end_orientation = get_end_orientation(red_start_orientation, red_motion['motion_type'], red_motion['turns'])
