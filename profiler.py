...
class Profiler:
    ...
    def write_profiling_stats_to_file(self, file_path: str, app_root: str) -> None:
        ...
        try:
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write("Organized by total time (all calls included):\n\n")
                self._write_stats_section(f, stats, "time", app_root, recursive=True)
                f.write("\n\n")
                f.write("Organized by number of calls (all calls included):\n\n")
                self._write_stats_section(f, stats, "calls", app_root, recursive=True)
                f.write("\n\n")
                f.write("Organized by cumulative time (all calls included):\n\n")
                self._write_stats_section(
                    f, stats, "cumulative", app_root, recursive=True
                )
        finally:
    ...
    def _write_stats_section(
        ...
        filtered_stats = stats.stats.items()
        if not recursive:
            filtered_stats = [
                (func, stat_info)
        ...
        sorted_stats = sorted(
            ...
            key=lambda x: (
                x[1][2]
                if sort_by == "time"
                else (x[1][0] if sort_by == "calls" else x[1][3])
            ),
            reverse=True,
        )
