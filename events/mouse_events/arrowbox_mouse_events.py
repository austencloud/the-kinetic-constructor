...
class ArrowBoxMouseEvents():
    ...
    def handle_drag_inside_graphboard(self, view, event):
        ...
        local_pos_in_graphboard = self.graphboard_view.mapFrom(view.window(), view.mapTo(view.window(), event.pos()))
    ...
    def rotate_drag_preview(self):
        renderer = QSvgRenderer(self.dragged_arrow.svg_file) # dragged_arrow is the arrow object that we picked up from the arrowbox
        ...
        angle = self.dragged_arrow.get_rotation_angle(self.drag_preview.quadrant, self.drag_preview.motion_type, self.drag_preview.rotation_direction)
        ...
        self.drag_preview.start_location, self.drag_preview.end_location = self.dragged_arrow.attributes.get_start_end_locations(self.drag_preview.motion_type, self.drag_preview.rotation_direction, self.drag_preview.quadrant)
    ...
    def handle_mouse_release(self, view, event, drag_preview):
        ...
        if not over_graphboard and self.has_entered_graphboard_once:
            ...
            new_arrow = self.graphboard_view.arrow_factory.create_arrow(self.graphboard_view, new_arrow_dict)
            ...
            new_arrow.arrow_manager.arrow_positioner.update_arrow_position(self.graphboard_view)
